# Restore S3 Deleted Objects

- [Restore S3 Deleted Objects](#restore-s3-deleted-objects)
  - [1 - Restore accidentally deleted files from Amazon S3](#1---restore-accidentally-deleted-files-from-amazon-s3)
    - [When should you use this script?](#when-should-you-use-this-script)
...
- Restore files from all buckets:
  ...
  [This script](./revive_s3_objects_from_all_buckets.py) iterates through all of the files in all of the buckets in an AWS account. If the files have been deleted after a specific point in time (`point_of_restore`), then the script restores the files to their latest state.

- Restore files from one specific bucket:

  [This script](./revive_s3_objects_from_1_bucket.py) iterates through all of the files in a specific bucket/prefix in an AWS account. If the files have been deleted after a specific point in time (`point_of_restore`), then the script restores the files to their latest state.
...
2. Edit the chosen script to set the `point_of_restore` variable. This should represent the date and time of the incident. Make sure to round down, so we don't miss any files.
3. If you chose the script that restores from one specific bucket, set the name of the bucket (`bucket_name`) as well
4. If you want to restore a specific prefix in the bucket, set the name of the prefix (`prefix`) as well. An empty prefix will restore all files from bucket.
5. If you need a list of all of the objects restored, set `list_all_restored_objects` to `True`.
6. If you want to log to a file, set the `filename` in `logging.basicConfig`.
7. If you feel more comfortable running Jupyter Notebooks, a `.ipynb` file has also been provided.

---

### How to run this script

In order for you to run this Python 3 script, you will need a User with the permissions described in [this IAM policy](./policy-s3-restore-deleted-objects.json).
...
* b. Clone this GitHub repository
  ...
  ```shell
  git clone https://github.com/CMCuritiba/restore-s3-deleted-objects.git
  ```
...
- [Original restore-s3-deleted-objects repo](https://github.com/aws-samples/restore-s3-deleted-objects)
...
- [Command Line S3 Client and Backup for Linux and Mac](https://s3tools.org/s3cmd)
